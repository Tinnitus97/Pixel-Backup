...
public class ImportFileBuilderTests
{
    ...
    public async Task BuildAsync_CreatesVCardFromTheContactExport()
    {
        ...
        try
        {
            ...
            var created = await ImportFileBuilder.BuildAsync(directory, entries);
            ...
            var vcf = await File.ReadAllTextAsync(Path.Combine(dataDirectory, "kontakte.vcf"));
    ...
    public async Task BuildAsync_IgnoresExportsWithoutUsableRows()
    {
        ...
        try
        {
            ...
            Assert.Empty(await ImportFileBuilder.BuildAsync(directory, entries));
        }
        finally
        {
            Directory.Delete(directory, recursive: true);
